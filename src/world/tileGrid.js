 ...
 * non-negative multiple of this. One step = one tile (1.5m) so step edges
 * align with the horizontal grid — a ramp is a single tile that bridges one
 * step. Consumers that need "one floor up" should use `LAYER_HEIGHT`.
 */
export const TERRAIN_STEP = TILE_SIZE;
/** Height of one Z-layer in world units (3m) — two terrain steps tall. */
export const LAYER_HEIGHT = TERRAIN_STEP * 2;
/** Maximum terrain step count generated by `generateTerrain`. 0..this inclusive. */
const MAX_TERRAIN_STEPS = 4;
...
export class TileGrid {
   ...
   * `TERRAIN_STEP` (1.5m) increments up to `MAX_TERRAIN_STEPS`.
   */
  generateTerrain() {
    const bands = 8;
