/**
 * Transient speech bubbles above cows — the social system writes a `Chat`
 * component with `text` + `expiresAtTick`, and this renderer shows it until
 * the current sim tick passes that expiry. Parked above the thought bubble
 * (which in turn sits above the name tag) so the three stack cleanly.
 *
 * Shares the texture cache + LRU pattern with cowThoughtBubbles — chat
 * phrases are drawn from a ~40 verb × ~60 topic pool so the same composed
 * line repeats often enough for caching to pay off.
 */

import * as THREE from 'three';
import { UNITS_PER_METER } from '../world/coords.js';

const HEAD_OFFSET = 4.2 * UNITS_PER_METER;
const BUBBLE_HEIGHT_WORLD = 0.5 * UNITS_PER_METER;
 ...
 */
export function createCowChatBubbles(scene) {
  /**
   ...
   * @param {number} alpha
   * @param {number} simTick
   */
  function update(world, camera, alpha, simTick) {
    camera.getWorldPosition(_camPos);
    camera.getWorldDirection(_camFwd);

    alive.clear();
    for (const { id, components } of world.query(['Cow', 'Position', 'Chat'])) {
      const chat = components.Chat;
      if (!chat.text || simTick >= chat.expiresAtTick) continue;
      alive.add(id);
      let bubble = bubbles.get(id);
      if (!bubble || bubble.text !== chat.text) {
        if (bubble) disposeBubble(scene, bubble, textureCache);
        bubble = makeBubble(scene, chat.text, textureCache);
...
function renderTextToCanvas(text) {
  ...
  measureCtx.font = `italic ${fontPx}px system-ui, -apple-system, Segoe UI, sans-serif`;
  ...
  // Cream-on-darkblue pill distinguishes chat from the darker thought bubble
  // so players can tell at a glance whether a cow is thinking or talking.
  const radius = height / 2;
  ctx.fillStyle = 'rgba(248, 242, 220, 0.92)';
  ctx.beginPath();
  ctx.moveTo(radius, 0);
  ...
  ctx.font = `italic ${fontPx}px system-ui, -apple-system, Segoe UI, sans-serif`;
