/**
 * World-space progress bars above any active crafting station (furnace, easel,
 * stove). Two camera-facing Sprites per active station: a dark background and
 * a coloured fill whose x-scale tracks `1 - workTicksRemaining / recipe.workTicks`.
 * Sprites use `center` at the left edge so the fill extends rightward from a
 * fixed origin rather than scaling from its middle.
 *
 * Idle stations get no sprite — they're created on craft start and torn down
 * when `activeBillId` flips back to 0 or the station despawns.
 */

import * as THREE from 'three';
import { UNITS_PER_METER, tileToWorld } from '../world/coords.js';
import { RECIPES } from '../world/recipes.js';
import { EASEL_HEIGHT } from './easelInstancer.js';
import { FURNACE_HEIGHT } from './furnaceInstancer.js';
import { STOVE_BODY_HEIGHT } from './stoveInstancer.js';

const BAR_WIDTH = 0.9 * UNITS_PER_METER;
const BAR_HEIGHT = 0.14 * UNITS_PER_METER;
const BAR_CLEARANCE = 0.35 * UNITS_PER_METER;
const FADE_START = 25 * UNITS_PER_METER;
const FADE_END = 110 * UNITS_PER_METER;

// Per-kind vertical offset: bar floats above the silhouette of each body.
// Furnace adds a little extra so it clears the chimney cap.
const BAR_Y_BY_COMP = {
  Furnace: FURNACE_HEIGHT + BAR_CLEARANCE + 0.55 * UNITS_PER_METER,
  Easel: EASEL_HEIGHT + BAR_CLEARANCE,
  Stove: STOVE_BODY_HEIGHT + BAR_CLEARANCE + 0.55 * UNITS_PER_METER,
};

const STATION_COMPS = /** @type {const} */ (['Furnace', 'Easel', 'Stove']);
 ...
 */
export function createStationProgressBars(scene) {
  /**
   * @type {Map<number, {
   ...
   */
  const bars = new Map();
  // Reused each frame so we're not allocating a Set per render pass.
  const alive = new Set();
  let visible = true;
  ...
  function update(world, grid, camera) {
    ...
    alive.clear();
    for (const comp of STATION_COMPS) {
      for (const { id, components } of world.query([comp, 'TileAnchor', 'Bills'])) {
        const station = components[comp];
        if (station.activeBillId <= 0) continue;
        const bill = components.Bills.list.find((b) => b.id === station.activeBillId);
        if (!bill) continue;
        ...
        const y = grid.getElevation(a.i, a.j) + BAR_Y_BY_COMP[comp];
        let bar = bars.get(id);
        if (!bar) {
          bar = makeBar(scene);
          // Stations don't move after placement, so the position is written
          // once at creation rather than every frame — matrix dirty flag
          // cost was non-trivial in the previous version.
        ...
        const progress = Math.max(
          0,
          Math.min(1, 1 - station.workTicksRemaining / recipe.workTicks),
        );
